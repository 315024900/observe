...
function getOn(element, ObData) {
    ...
    atts.filter(obj => obj.name[0] === '@').forEach((obj) => {
        ons[obj.name.replace('@', '')] = function (e) {
            ...
            let $event = {};
            ['type', 'target'].forEach(key => $event[key] = e[key])
            new Function(`$event,` + Object.keys(ObData).join(','), obj.value)($event, ...Object.keys(ObData).map(key => ObData[key]));
        };
    });
    return ons;
}
    ...
    const createNode = elment => h(elment.tagName.toLowerCase(),
        {
            ...
            // 正常的 HTML 特性
            attrs: elment.getAttributeNames().reduce((obj, key) => obj[key] = elment.getAttribute(key)),
